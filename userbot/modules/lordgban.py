...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit("`Boss, Ini Tidak Mungkin Tanpa ID Pengguna`")
            return
                ...
                if i.sender == str(guser.id):
                    ...
                    if admin or creator:
                        try:
                            ...
                            await tele.reply(
                                f"**My Boss, Pengguna Gban Telah Bergabung** \n"
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Virus Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini ヅ`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Virus ヅ`")
    ...
    if user:
        if user.id == 1370683503:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke My Boss Tungau, Dia Adalah Pembuat Saya ヅ`"
            )
        try:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Virus`")
    try:
        if gmute(user.id) is False:
            return await dark.edit(f"**Kesalahan! Pengguna Ini Sudah Kena Perintah Global Banned Virus.**")
    except BaseException:
        pass
    return await dark.edit(
...
async def gunben(userbot):
    ...
    if user:
        ...
        await dark.edit("`Harap Balas Ke Pesan Pengguna Virus ヅ`")
    try:
